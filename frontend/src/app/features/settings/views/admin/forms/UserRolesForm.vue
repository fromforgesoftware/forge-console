...
const props = defineProps<{ user: AdminUser; roles: Role[]; canWrite: boolean }>();
...
async function submit(): Promise<void> {
	...
	error.value = '';
	try {
		await adminService.setUserRoles(props.user.id, selected.value);
		emit('saved');
	} catch {
...
onMounted(async () => {
	try {
		const assigned = await adminService.getUserRoles(props.user.id);
